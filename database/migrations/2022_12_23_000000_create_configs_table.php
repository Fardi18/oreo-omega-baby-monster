...
class CreateConfigsTable extends Migration
{
    ...
    public function up()
    {
        Schema::create(env('PREFIX_TABLE') . 'configs', function (Blueprint $table) {
            ...
            $table->enum('app_skin', ['default','festive_yellow', 'feminine_purple', 'racing_red', 'calm_blue', 'simple_black', 'manly_maroon', 'clean_blue', 'confidence_blue'])->default('calm_blue');
            $table->text('bg_images')->nullable()->comment('JSON array formatted');
            ...
            $table->text('recaptcha_site_key_admin')->nullable();
            $table->text('recaptcha_secret_key_admin')->nullable();
            $table->text('recaptcha_site_key_public')->nullable();
            $table->text('recaptcha_secret_key_public')->nullable();

            $table->boolean('secure_login')->default(1);
            $table->tinyInteger('login_trial')->default(3);
