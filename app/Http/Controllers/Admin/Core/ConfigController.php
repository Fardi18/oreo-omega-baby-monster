...
class ConfigController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($request->isMethod('post')) {
            ...
            try {
                ...
                // IF DELETE FILE
                if ($request->app_logo_delete == 'yes') {
                    $result = Helper::delete_file($data->app_logo, true);
                    $data->app_logo = 'images/logo-square.png';
                }
                ...
                if ($request->file('app_logo')) {
                    ...
                    $image = Helper::upload_file($dir_path, $image_file, true, $format_image_name);
                    if ($image['status'] != true) {
                        return back()
                            ->withInput()
                            ->with('error', lang($image['message'], $this->translations));
                    }

                    // GET THE UPLOADED IMAGE RESULT
                    $data->app_logo = $image['uploaded_full_path'];
                }

                $app_logo_style = Helper::validate_input_text($request->app_logo_style);
                if (!in_array($app_logo_style, ['logo with app name', 'logo only'])) {
                    return back()
                        ->withInput()
                        ->with('error', lang('Invalid format for #item', $this->translations, ['#item' => ucwords(lang('application logo style', $this->translations))]));
                }
                $data->app_logo_style = $app_logo_style;

                $login_page_style = Helper::validate_input_text($request->login_page_style);
                if (!in_array($login_page_style, ['animated', 'default'])) {
                    return back()
                        ->withInput()
                        ->with('error', lang('Invalid format for #item', $this->translations, ['#item' => ucwords(lang('login page style', $this->translations))]));
                }
                $data->login_page_style = $login_page_style;
    ...
    public function import(Request $request)
    {
        ...
        if ($file->move($destination_path, $filename)) {
            ...
            return redirect()
                ->route('admin.config')
                ->with('success', lang('Successfully imported #item data', $this->translations, ['#item' => $this->item]));
        }

        // FAILED
        return back()
            ->withInput()
            ->with('error', lang('Oops, failed to import #item data. Please try again.', $this->translations, ['#item' => $this->item]));
    }
}
