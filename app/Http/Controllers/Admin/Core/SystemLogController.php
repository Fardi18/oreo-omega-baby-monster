...
class SystemLogController extends Controller
{
    ...
    public function get_data(Datatables $datatables, Request $request)
    {
        ...
        $daterange = Helper::validate_input_text($request->daterange);
        if (!empty($daterange)) {
            $arr_date = explode(' - ', $daterange);
            if (count($arr_date) > 0) {
                $from_date = Helper::convert_datepicker($arr_date[0]) . ' 00:00:00'; // APP_TIMEZONE
                $from_date = Helper::server_timestamp($from_date); // convert from APP_TIMEZONE to SERVER_TIMEZONE coz DB using SERVER_TIMEZONE
                $to_date = Helper::convert_datepicker($arr_date[1]) . ' 23:59:59'; // APP_TIMEZONE
                $to_date = Helper::server_timestamp($to_date); // convert from APP_TIMEZONE to SERVER_TIMEZONE coz DB using SERVER_TIMEZONE
                $query->whereBetween($table_log . '.created_at', [$from_date, $to_date]);
            }
        }
    ...
    public function export(Request $request)
    {
        ...
        $daterange = Helper::validate_input_text($request->daterange);
        if (!empty($daterange)) {
            $filename = $this->module . ' (' . $daterange . ').csv';
        ...
        if (!empty($data[0])) {
            ...
            $callback = function () use ($data) {
                ...
                // SET HEADER FIELDS
                $header_fields = [
                    ucwords(lang('log ID', $this->translations)),
                    ucwords(lang('user', $this->translations)),
                    ucwords(lang('activity', $this->translations)),
                    lang('IP Address', $this->translations),
                    strtoupper(lang('url', $this->translations)),
                    ucwords(lang('user agent', $this->translations)),
                    ucwords(lang('timestamp', $this->translations)),
                    ucwords(lang('datetime', $this->translations))
                ];
                fputcsv($fh, $header_fields);

                foreach ($data as $item) {
                    $final_array    = [];
                    $final_array[]  = $item->id;
                    $final_array[]  = $item->username;
                    ...
                    $final_array[]  = implode(' ', $arr_log_label); // activity

                    $final_array[]  = $item->ip_address;
                    $final_array[]  = $item->url;
                    $final_array[]  = $item->user_agent;
                    $final_array[]  = Helper::locale_timestamp($item->created_at, 'Y-m-d H:i:s', false);
                    $final_array[]  = Helper::locale_timestamp($item->created_at);

                    fputcsv($fh, $final_array);
                }
            ...
            # FAILED (NO DATA)
            return redirect()->route('admin.system_logs')->with('error', 'No Data');
        }
    }
}
