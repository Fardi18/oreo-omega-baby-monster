...
class AuthController extends Controller
{
    ...
    public function login()
    {
        ...
        if ($this->global_config->login_page_style == 'animated') {
            return view('admin.core.login_v2');
        } 
    ...
    public function login_auth(Request $request)
    {
        ...
        try {
            ...
            // google 2fa
            $obj->google2fa_secret = $data->google2fa_secret;
            $obj->google2fa_enabled = $data->google2fa_enabled;
            $obj->google2fa_passed = false;
            if ($data->google2fa_enabled && empty($data->google2fa_secret)) {
                $obj->google2fa_passed = true;
            }
    ...
    public function verify_2fa(Request $request)
    {
        ...
        if ($request->isMethod('post')) {
            ...
            if (empty($admin)) {
                return back()
                    ->withInput()
                    ->with('error', lang('Admin data not found', $this->translations));
            }

            // Cek if 2FA is enabled
            ...
            if ($google2fa->verifyKey($admin->google2fa_secret, $code)) {
                // Update session 2FA verified
                $admin_session = Session::get(env('SESSION_ADMIN_NAME'));
                $admin_session->google2fa_passed = true;

                // Refresh session
                Session::put(env('SESSION_ADMIN_NAME', 'sysadmin'), $admin_session);

                return redirect()->route('admin.home');
            } else {
                return back()
                    ->withInput()
                    ->with('error', lang('2FA code not valid', $this->translations));
            }
        } else {
            $global_config = $this->global_config;

            return view('admin.core.login_2fa', compact('global_config'));
        }
    }
