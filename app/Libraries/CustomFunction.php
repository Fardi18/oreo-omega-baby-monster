    ...
    function set_input_form($type, $input_name, $label_name, $data, $errors, $required = false, $config = null)
    {
        ...
        switch ($type) {
            ...
            case 'number_only':
                ...
                $input_element = '<input type="number" value="' . $value . '" ' . $properties . ' class="form-control col-md-7 col-xs-12" onkeyup="numbers_only(this);" />';
                if ($limit_chars > 0 && !isset($input_addon)) {
            ...
            case 'image':
                ...
                } elseif ($model == 'no button') {
                    $input_element = '<img id="vimg_preview_' . $input_name . '" src="' . $img_src . '" style="max-width:200px; cursor:pointer;" data-toggle="tooltip" data-placement="right" title="click image to browse" onclick="$(\'#' . $id_name . '\').click()" />';
                    $input_element .= '<input type="file" name="' . $input_name . '" id="' . $id_name . '" accept="image/*" onchange="readURL(this, \'before\', \'vimg_preview_' . $input_name . '\', \'' . $no_image . '\', \'' . $img_src . '\');" style="display: none" />';
                } else {
            ...
            case 'multiple_images':
                ...
                for ($i = 0; $i < $total_items; $i++) {
                    if (isset($value[$i])) {
                        // set image using "$value" only
                        $img_src = asset($value[$i]);
                        if (isset($path)) {
                            // set image using "$path" & "$value"
                            $img_src = asset($path . $value[$i]);
                        }
                    ...
                    $input_element .= '<img id="vimg_preview_image_multi_' . $input_name . '_' . $i . '" src="' . $img_src . '" style="max-width:200px; cursor:pointer; margin:5px;" data-toggle="tooltip" data-placement="right" title="click image to browse" onclick="$(\'#image_multi_' . $input_name . '_' . $i . '\').click()" />';
                    $input_element .= '<input type="file" name="' . $input_name . '[' . $i . ']" id="image_multi_' . $input_name . '_' . $i . '" ' . $required_status . ' accept="image/*" onchange="readURL(this, \'before\', \'vimg_preview_image_multi_' . $input_name . '_' . $i . '\', \'' . $no_image . '\', \'' . $img_src . '\');" style="display:none" />';
                }
                if (isset($info)) {
                    $input_element .= '<br><span>' . $info . '</span>';
                }
        ...
        if (isset($input_addon)) {
            $element .= '<div class="input-group" style="margin-bottom: 0 !important;">';
            ...
            if ($limit_chars > 0) {
                ...
                $element .= '<span><b id="limit_chars_' . $input_name . '">' . $available_chars . '</b>/' . $limit_chars . '</span>';
            }
        } else {
            $element .= $input_element;
        }
