...
class TheHelper
{
    ...
    public static function validate_input_email($string)
    {
        ...
        // Check the email against a regular expression pattern for stricter validation
    ...
    public static function validate_date($string, $format = 'Y-m-d H:i:s', $strict = true)
    {
    ...
    public static function convert_datepicker($inputdate, $delimiter = '/', $glue = '-', $order = '210')
    {
        if (!empty($inputdate)) {
            ...
            return [
                'status' => true,
                'message' => 'OK',
                'data' => $date_arr[$order_arr[0]] . $glue . $date_arr[$order_arr[1]] . $glue . $date_arr[$order_arr[2]]
            ];
        }
        return [
            'status' => false,
            'message' => 'NO INPUT DATE',
            'data' => ''
        ];
    }
    ...
    {
        $ch = curl_init('https://unshorten.me/json/' . urlencode($url));
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
    ...
    public static function remove_directory($dir)
    {
        ...
        foreach (scandir($dir) as $item) {
            ...
            if (!TheHelper::remove_directory($dir . DIRECTORY_SEPARATOR . $item)) {
                return false;
            }
        }
    ...
    public static function validate_forbidden_word($message, $forbidden_word = ['fuck', 'stupid'])
    {
        if (empty($message)) {
            # FAILED
            return [
        ...
        foreach ($words as $word) {
            if (in_array($word, $forbidden_word)) {
                $find_forbidden_word = $word;
                break;
            }
        }
        if ($find_forbidden_word) {
            # FAILED
            return [
                'status' => 'false',
                'message' => 'Your message contains forbidden word (' . $find_forbidden_word . ')',
                'data' => $find_forbidden_word
            ];
        } else {
            # SUCCESS
            return [
    ...
    public static function convert_microtime($duration)
    {
    ...
    public static function get_country_flag_image($country_code_two_letter)
    {
